...
Если сеть отсутствует, приложение может создать её автоматически, но в compose-файле она всё равно должна быть объявлена как `external: true` с именем `soul-dialogue`, иначе окружение будет работать некорректно или деплой не запустится.
